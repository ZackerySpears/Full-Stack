...
constructor(props) {
  ...
  componentDidMount() {
  this.getDataFromDB();
  //Set up an interval to call the getDataFROMDB function from every second
  ...
  postDataToDB = message => {
    //1. Fiqure out what ID this message needs to have 
  ...
  renderListItems() {
    ...
    if (data.length === 0) {
      //If our data array doesn't have any contentsm, display a message letting the user know that the database doesn't have anything inside it yet  
      return "NO BD ENTRIES YET";
    } else {
      return data.map(dat => (
        <li key={dat}>
          <span> id: {dat.id} </span>
